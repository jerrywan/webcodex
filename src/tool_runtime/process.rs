...
impl ToolRuntime {
    ...
    pub(super) async fn process_shell_recovery_call(
        ...
        let super::ToolCall::RunProcess {
            project, executable, args, stdin, session_id, timeout_secs,
            sync_wait_secs, cwd, purpose,
        } = call else { return None; };
        if !matches!(executable.as_str(), "sh" | "bash")
            || args.len() != 2 || args[0] != "-c" || stdin.is_some()
            || cwd.as_ref().is_some_and(|cwd| cwd.len() > PROCESS_CWD_MAX_BYTES || cwd.contains('\0'))
            || ssh_resource.is_some() || !expectation.accepted_exit_codes.is_empty()
            || expectation.expected_failure || expectation.expected_failure_kind.is_some()
            || expectation.result_expectation.as_deref().is_some_and(|value| value != "success")
        {
            return None;
        }
        let process = ShellProcessArgv { executable: executable.clone(), args: args.clone() };
        if validate_process_input(&process, None, cwd.as_deref()).err().as_deref()
            != Some("run_process does not accept shell command modes; use run_shell for shell syntax")
        {
            return None;
        }
        super::helpers::explicit_shell_dispatch_command(&args[1], executable).ok()?;
        let resolved = resolved?;
        let runner = self.runner_registry.get_runner_view(&resolved.config.client_id).await?;
        ...
        let selected_profile = entry.shell_profile.as_deref().or(profiles.default_profile.as_deref());
        let dialect = match selected_profile {
            Some(name) => profiles.profiles.iter().find(|profile| profile.name == name)?.dialect.as_deref(),
            None => profiles.default_dialect.as_deref(),
        };
        if !matches!(dialect, Some("sh" | "bash")) { return None; }
        let mut arguments = json!({"project": project, "shell": executable, "command": args[1]});
        for (name, value) in [
            ("session_id", json!(session_id)), ("cwd", json!(cwd)),
            ("timeout_secs", json!(timeout_secs)), ("sync_wait_secs", json!(sync_wait_secs)),
            ("purpose", json!(purpose)), ("assertion_name", json!(expectation.assertion_name)),
            ("result_expectation", json!(expectation.result_expectation)),
        ] {
            if !value.is_null() { arguments[name] = value; }
        }
