...
async fn observe_jobs_terminal_policy_coalesces_noisy_jobs_with_one_deadline_and_caller_delta() {
    ...
    if policy == ObserveJobsWakeOn::AllTerminal {
        update_observed_job(&runtime, "coalesce-a", &request_a, "completed", None, None, true).await;
    ...
    assert_eq!(
        a["activity"],
        if policy == ObserveJobsWakeOn::AllTerminal { Value::Null } else { serde_json::to_value(process_activity()).unwrap() }
    );
}
}
...
async fn observe_jobs_updates_around_first_wait_registration_are_not_lost() {
    ...
    for policy in [ObserveJobsWakeOn::Change, ObserveJobsWakeOn::Terminal, ObserveJobsWakeOn::AllTerminal] {
...
async fn observe_jobs_all_terminal_waits_for_entire_set_and_preserves_original_delta() {
    for initially_terminal in [0, 1, 2] {
        ...
        update_observed_job(&runtime, "all-a", &request_a,
            if initially_terminal > 0 { "completed" } else { "running" },
            Some("before wait\n"), None, initially_terminal > 0).await;
        if initially_terminal == 2 {
            update_observed_job(&runtime, "all-b", &request_b, "completed", Some("b done\n"), None, true).await;
        }
        // Exactly one outer observation, including both staggered completions.
        let observation = runtime.observe_jobs_for_auth(
            vec![item(&a, Some(token_a)), item(&b, Some(token_b))],
            40, Some(5), ObserveJobsWakeOn::AllTerminal, Some(&auth));
        tokio::pin!(observation);
        if initially_terminal < 2 {
            assert!(futures_util::poll!(&mut observation).is_pending());
            if initially_terminal == 0 {
                update_observed_job(&runtime, "all-a", &request_a, "completed", Some("a done\n"), None, true).await;
                assert!(futures_util::poll!(&mut observation).is_pending(), "first terminal Job must not wake the batch");
            }
            update_observed_job(&runtime, "all-b", &request_b, "completed", Some("b done\n"), None, true).await;
        }
        let result = tokio::time::timeout(Duration::from_secs(1), observation).await.unwrap();
        assert!(result.success, "{:?}", result.error);
        assert_eq!(result.output["wait"]["outcome"], "terminal");
        ...
        assert_eq!(result.output["items"][0]["output"]["stdout_tail"],
            if initially_terminal == 0 { "before wait\na done\n" } else { "before wait\n" });
        if initially_terminal == 2 {
            assert_eq!(result.output["wait"]["waited_ms"], 0);
        }
    }
...
async fn observe_jobs_all_terminal_item_errors_return_without_waiting_for_running_job() {
    ...
    for (bad_item, caller) in [
        ...
        let result = tokio::time::timeout(Duration::from_secs(1), runtime.observe_jobs_for_auth(
            vec![item(&job, Some(token.clone())), bad_item], 40, Some(5),
            ObserveJobsWakeOn::AllTerminal, Some(caller))).await.unwrap();
        assert!(result.success, "{:?}", result.error);
        assert_eq!(result.output["wait"]["outcome"], "item_error");
