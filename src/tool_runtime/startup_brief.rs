...
pub(crate) fn builtin_coding_workflow_projection() -> Value {
    json!({
        ...
        "model_protocol": {
            "handoff_recovery": "Use session_handoff_summary only when task context is actually missing after compaction/restart, an explicit cross-window/Agent handoff needs broader retained evidence, or the user asks to recover prior task state. Do not call it routinely to check progress, establish a baseline, or refresh an otherwise coherent Session; use direct current-state observations instead. Requires the exact session_id; check basis completeness before dependent work.",
