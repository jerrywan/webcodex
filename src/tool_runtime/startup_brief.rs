...
pub(crate) fn builtin_coding_workflow_projection() -> Value {
    json!({
        ...
        "guidance": [
            ...
            "Validation failure is evidence, not queue cleanliness. Fix blockers before dependent work; otherwise continue independent work. Reuse assertion_name on rerun; mutation stales evidence; outcome_unknown fails closed. Formatting is finalization, not per-edit validation.",
            "Long work keeps one execution/Job and exact continuation; wait_secs=100,wake_on=terminal only when blocked, not for visibility. Format once after Rust source stabilizes, before final diff/closeout; rerun only after later Rust edits affecting formatting. Final source needs sufficient fresh validation."
        ],
        "model_protocol": {
