...
pub(crate) fn builtin_coding_workflow_projection() -> Value {
    json!({
        ...
        "guidance": [
            ...
            "Choose the mutation method with the highest expected correctness and reliability, not by preferred-tool usage. Use apply_text_edits for small precise local edits when exact targets are natural; use write_project_file for intentional create/whole-file replacement; apply_patch remains available when patch-shaped input is genuinely clearest, not as default recovery. A bounded deterministic Python transformation through run_shell is a first-class option for repetitive, multi-file, or programmatic source rewrites when simpler and less error-prone; keep it tightly scoped, do not bypass permission/path policy, and avoid network unless required and authorized. Always inspect the resulting diff and validate final source. After source edits or a rustfmt check diff, prefer cargo_fmt(check=false) to ensure formatting instead of reproducing rustfmt edits manually; apply model_protocol only where the exposed schema supports it.",
