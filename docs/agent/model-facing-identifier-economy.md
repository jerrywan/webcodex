...
| Conversation, conversation Message, Delivery, attention event | domain prefix + 32 hex → B64(12) | store/communication, agent_attention; SQLite primary keys | communication/task continuation projections |
...
| Connector Task, Run, Result and checkpoint identities | domain prefix + 32 hex; audit before changing | connector-runtime, store/task_kernel, tool_runtime/checkpoint | verify actual model projection and owning uniqueness boundary |
