...
const state = {
  project: { path: "/fixture/alpha", allowed_root: "/fixture/alpha", is_git_repository: false },
...
const onState = vi.fn();
const wrap = (element: React.ReactNode) => <LocaleProvider>{element}</LocaleProvider>;
...
describe("workspace configuration boundaries", () => {
  it("edits running Tunnel credentials and clears the write-only key", async () => {
    ...
    const id = screen.getByLabelText("Tunnel ID"), key = screen.getByLabelText("Tunnel API key");
  ...
  it("saves exact-target instruction and Skill paths and fences Runner restart", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Save Runner settings" }));
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "Restart Desktop-owned Runner" })).not.toBeDisabled());
    fireEvent.click(screen.getByRole("button", { name: "Restart Desktop-owned Runner" }));
  ...
  it("keeps permission probe failures visible and allows an explicit retry", async () => {
    ...
    expect(screen.getByText("Not observed here")).toBeInTheDocument();
    expect(api.requestComputerPermission).not.toHaveBeenCalled();
  });
});
