...
export function ConnectionCard({ profile, canStart, busy, copied, onAction, onEdit, onDelete, onCopy }: {
  ...
  return <article className="connection-profile" aria-labelledby={`connection-${profile.id}`} data-tunnel-profile-id={profile.id}>
    ...
    <div className="connection-actions">
      ...
      <button type="button" className={active ? "secondary-button" : "primary-button"} disabled={busy || !canStart || !profile.credential_present} aria-label={`${p(active ? "restart" : "start")} ${profile.name}`} onClick={() => onAction(active ? "restart" : "start")}>{p(active ? "restart" : "start")}</button>
      {active && <button type="button" className="secondary-button" disabled={busy} aria-label={`${p("stop")} ${profile.name}`} onClick={() => onAction("stop")}>{p("stop")}</button>}
    ...
    <details className="connection-details"><summary>{p("advanced")} · {profile.name}</summary>
      ...
      <dl><dt>Profile ID</dt><dd><code>{profile.id}</code></dd><dt>PID</dt><dd>{profile.pid ?? "—"}</dd>{profile.local_mcp_url && <><dt>MCP</dt><dd><code>{profile.local_mcp_url}</code></dd></>}{profile.runtime_directory && <><dt>Runtime</dt><dd><code>{profile.runtime_directory}</code></dd></>}{profile.health_url && <><dt>Health</dt><dd><code>{profile.health_url}</code></dd></>}{profile.log_file && <><dt>Log</dt><dd><code>{profile.log_file}</code></dd></>}</dl>
