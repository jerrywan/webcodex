...
pub(super) const DEFINITIONS: &[ToolDefinition] = &[
        ...
        adaptive_runtime_direct(
            permission_risk(
                model_spec(
                    ...
                    "Create one caller-owned durable one-shot AgentWait over 1..8 exact agent_task_terminal selectors with closed mode any|all. Mode defaults to any. Under any, the first match triggers and later matches may coalesce only while the same Wake is pending/claimed. Under all, partial matches remain durable without creating a Wake; only the final required source match triggers one Wake. Re-authorizes the exact Agent, current Endpoint generation, and every source Task before atomically registering sources and already-terminal snapshots. The Endpoint is presentation only, not durable Wait ownership. Exact keyed replay returns the same Wait.",
    ...
    require_all_scopes(
        model_spec(
            ...
            "Read one exact caller-owned AgentWait with bounded self-contained rendezvous truth: wait_id, state, mode, source_count, match_count, all registered source Task identities, and matching Task/Attempt identities with terminal states. Source references contain only kind/task_id. The result contains no source Task instruction/result/reason/log/fence/token, Project reference, Endpoint, or credentials and grants no Task, Project, Goal, Session, or execution authority; re-read source Tasks independently.",
        ),
