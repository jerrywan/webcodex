...
pub(super) const DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Canonical bootstrap for ordinary coding/review. Prefer a Server-issued project_ref; use project or client_id+path, and canonical Project ids remain accepted. Omit session_id for a fresh Workflow Session; it does not imply a fresh model context. Exact resume requires an active accessible Session and never guesses prior Session. Runtime always re-observes instruction files for change detection and Session metadata, while primary result stays compact. For a fresh or uncertain model context, request missing guidance via context_request: project.instructions and/or webcodex.workflow; omission projects neither. guidance_profile selects direct/code_mode model guidance only and grants no authority or Session state. project_ref is durable principal-scoped convenience, not authority; each use reauthorizes the canonical Project. include_extension_catalog controls the bounded Skills/Plugins catalog. Checkout does not require Git; mode=worktree uses an exact Git base without bypassing Project authority.",
