...
pub(super) const DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        context_reobservable(model_spec(
            ...
            "Canonical bootstrap for ordinary coding/review. Use project or client_id+path. Omit session_id for a fresh Workflow Session; supply it for exact resume of an active accessible Session; never guesses prior Session. A fresh Workflow Session does not imply a fresh model context: when the current model context already retains project instructions and/or built-in workflow guidance, explicitly set the corresponding include_* flag false even while creating a new Workflow Session. Keep those defaults true for a fresh or uncertain model context. Session/window/transport/credential/Server identity never proves retention, and the Server never guesses it. include_project_instructions=false only suppresses redundant model-facing instruction bodies: Runtime still re-observes instruction files, fingerprints/change detection, and Session metadata. include_workflow_guidance=false only suppresses redundant built-in guidance projection. Defaults otherwise return bounded project instructions, workflow guidance, and Skills/Plugin selection metadata, which grants no authority. Skill bodies require skill_read_file; Plugin calls require plugin_tool describe. include_extension_catalog=false skips discovery. Checkout accepts Project or Runner path and does not require Git; mode=worktree resolves an exact Git base and manages an isolated worktree without bypassing Project authority.",
