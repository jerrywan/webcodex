...
pub(super) const DETAIL_DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        context_reobservable(change_summary_like(git_like(model_spec(
            ...
            "Targeted/paged diff review for worktree/cached or exact base/head ranges, with paths and scope/fence-bound opaque continuation. max_page_bytes controls the raw producer page (64 KiB default, bounded below ordinary Runner result retention); it is separate from the 512 KiB final model-facing result ceiling. Continuation is a compact opaque runtime token; copy it verbatim only through the returned parser-ready next_call and do not interpret it. An opaque continuation may identify either later logical diff records or the next complete-line fragment of one exact hunk. recovery.later_hunks.next_call remains later-record only, while recovery.current_hunk.next_call may use bounded refinement or a distinct exact hunk-fragment token. Fixed byte/line ceilings never advertise fake recovery. Read-only.",
