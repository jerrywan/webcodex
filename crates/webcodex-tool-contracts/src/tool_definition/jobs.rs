...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    adaptive_runtime_direct(
        model_spec(
            ...
            "Run one-shot executable with structured argv. Preferred route for one native executable with literal argv; Windows batch shims use the bounded Runner-owned quoting contract. Use run_shell only when shell semantics or a short tightly related command chain is required. Do not open a persistent shell merely to run several commands: local persistence is only for same-process cwd/env/exports/functions/umask state; one named SSH resource preserves remote state. New targets use ssh_resource onboarding; one-shot/no-persistence SSH remains valid. Long work stays Runner-owned in the same execution; timeout_secs defaults to 60 seconds and clamps at 7 days. If the native child must outlive the Runner across restart/upgrade/stop/replacement, use run_detached_process from the start; duration alone is not a reason to detach.",
    ...
    model_spec(
        ...
        "Run sh, bash, PowerShell, JavaScript, or TypeScript as Runner-owned script data. JavaScript is Node.js-backed .mjs ESM. TypeScript uses Node native erasable type stripping in .mts ESM, requires Node.js 22.6+, does not type-check, and rejects enum/transform-required syntax. Runner owns runtime selection/flags; WebCodex does not install npm dependencies, run tsc, or fall back to Bun/Deno/tsx. Relative ESM imports resolve from the Runner-owned temporary module. Prefer run_process for native argv, run_script for program-like scripts, and run_shell when shell grammar is required. Long work stays Runner-owned in the same execution / same Job, never restarted; timeout_secs defaults to 60 seconds and clamps at 7 days; script bodies never become shell command text. If a native child must outlive the Runner across restart/upgrade/stop/replacement, use run_detached_process from the start.",
    )
    .with_execution(super::ToolExecutionContract::new(
