...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Continue known Jobs by job_id; do not call list_jobs first. Pass observation_token unchanged as after_observation_token. Observe 1-8 Jobs with bounded deltas and isolated errors. No token gives an immediate baseline; no wait_secs gives an immediate observation. With tokens, bounded wait_secs (runtime max 100) uses wake_on=change by default. Prefer the exact parser-ready continuation returned by WebCodex. When constructing a model-facing terminal wait manually, use wait_secs=55,wake_on=terminal when useful progress is blocked on terminal outcome; explicit Runtime waits up to 100 seconds remain valid but may exceed an outer MCP Host deadline. If independent work remains, keep the exact continuation and observe later—do not poll for visibility. Use terminal when any terminal result unblocks progress; use all_terminal when every Job in a predetermined set is required. Updates do not extend the shared deadline; item errors return immediately. Timeout may include changed=true deltas. reset is bounded recovery. Never launches, retries, stops, or subscribes.",
        ).with_gpt_action_description("Continue known Jobs by job_id/token. Prefer the returned continuation; if constructing a model-facing wait manually, use wait_secs=55,wake_on=terminal only when progress is blocked on terminal outcome. Runtime accepts up to 100 seconds, but longer waits may exceed the outer Host deadline. Otherwise retain the exact continuation, continue independent work, and observe later. Tokens are observation cursors, never retry/execution authority."),
        80,
    ),
    adaptive_runtime_direct(
        model_spec(
