...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Run a bounded shell command or short related chain for shell semantics. Predetermined related observations may share one call; adaptive/result-dependent follow-ups stay sequential. Prefer run_process for literal argv; run_script handles program-like languages. A deterministic Python heredoc fits one coherent transformation; respect Project/path/permission policy, avoid unauthorized network, inspect diff, validate final source. Keep independent effects and failure/permission/validation boundaries separate (commit, push, deploy, restart). Persistent shell is only for same-process state or repeated commands on one named SSH resource. Long runtime keeps one Runner-owned execution: timeout_secs is total lifetime, sync_wait_secs is Job-handoff grace, later observe wait is one observation. Duration does not select the primitive. Use run_detached_process to outlive this Runner.",
