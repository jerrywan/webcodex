...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    ...
    adaptive_runtime_direct(
        model_spec(
            ...
            "Run one bounded shell command or short tightly related shell command chain. Use it for shell semantics such as &&, pipes, redirects, globbing, substitution, or one tightly related observation goal to reduce model/tool round trips. A bounded deterministic Python heredoc through python3 is a first-class programmatic source-transformation path when it is clearer and less error-prone than encoding many structured text edits; keep it tightly scoped, do not use it to bypass Project/path/permission policy, avoid network unless the task requires and authorizes it, then inspect the diff and validate the final source. run_script does not imply Python support: use run_shell for this heredoc path. Keep run_process preferred for one native executable with literal argv. Do not chain independent effects or failure/permission boundaries such as validation, commit, push, deploy, or restart. Use run_script for its supported program-like script languages. Persistent shell is only for same-process cwd/env/export/function/umask state or repeated commands on one named SSH resource. Longer shell work stays Runner-owned. If a native child must outlive the current Runner process across restart/upgrade/stop/replacement, use run_detached_process from the start; shell duration alone is not a reason to detach.",
