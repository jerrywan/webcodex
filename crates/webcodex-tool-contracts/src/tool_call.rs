...
impl ToolCall {
    ...
    pub fn project_mut(&mut self) -> Option<&mut String> {
        match self {
            ...
            | Self::CodeModeExecMutating { project, .. } => Some(project),
            ...
            | Self::FindReferences { project, .. }
            | Self::CallHierarchy { project, .. } => Some(project),
            #[cfg(feature = "workspace-checkpoints")]
            ...
            | Self::WorkspaceCheckpointDelete { project, .. } => Some(project),
            Self::WorkOnProject { project, .. } if !project.trim().is_empty() => Some(project),
            Self::FinishCodingTask { project, .. }
            | Self::PresentWorkResult { project, .. }
            | Self::WorkResultState { project, .. }
            | Self::ChangesFileDiff { project, .. }
            | Self::UpdateSessionContext { project, .. }
            | Self::ValidationSummary { project, .. } => Some(project),
            Self::SessionHandoffSummary { project, .. } => project.as_mut(),
            _ => None,
        }
    }
